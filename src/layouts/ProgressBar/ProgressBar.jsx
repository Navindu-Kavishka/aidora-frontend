        ...
        <div className="progress-bar-form-wrapper" style={{ padding: '0 0 20px 0', marginTop: '30px', marginBottom: '30px' }}>
          <form id="progress-bar-form">
            <ul id="progress-bar-list" style={{ display: 'flex', justifyContent: 'space-between', listStyleType: 'none', color: 'lightgrey', marginBottom: '30px', padding: 0 }}>
              {[1, 2, 3].map((s) => (
                <li key={s} id={`progressbar-step-${s}`} style={{ textAlign: 'center', width: '25%', position: 'relative', color: stepNumber >= s ? '#2F8D46' : 'lightgrey' }}>
                  <div className="progress-bar-step-circle" style={{ width: '50px', height: '50px', lineHeight: '45px', display: 'block', fontSize: '20px', color: '#ffffff', background: stepNumber >= s ? '#2F8D46' : 'lightgray', borderRadius: '50%', margin: '0 auto 10px auto' }}>{s}</div>
                  <strong>{stepNames[s - 1]}</strong>
                  {s < 3 && <div className="progress-bar-step-line" style={{ content: '', width: '100%', height: '2px', background: stepNumber >= s ? '#2F8D46' : 'lightgray', position: 'absolute', left: 0, top: '25px', zIndex: -1 }}></div>}
                </li>
              ))}
            </ul>
            <div className="progress-bar-background" style={{ width: '100%', backgroundColor: '#e0e0e0', borderRadius: '13px', overflow: 'hidden', marginBottom: '20px' }}>
              <div ref={progressBarRef} className="progress-bar-fill" style={{ height: '20px', backgroundColor: '#2F8D46', borderRadius: '13px', width: `${((stepNumber / steps) * 100)}%` }}></div>
            </div>
            {[1, 2, 3].map((s, index) => (
              <fieldset
                key={s}
                ...
                style={{
                  ...
                  margin: 0,
                  paddingBottom: '10px',
                  display: stepNumber === s ? 'block' : 'none',
                  position: stepNumber === s ? 'relative' : 'absolute'
                }}
              >
              </fieldset>
            ))}
